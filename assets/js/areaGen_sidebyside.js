...
function drawInstance(tab_num, instance_id, instance_num) {
...
function drawInitGrid(tab_num, area_id, grid_class) {
  ...
  var functional_area = document.querySelector(area_sel + ">p").innerHTML;
  ...
  for (var ts = 1; ts <= max_ts; ts++) {
    for (var grid_idx = 0; grid_idx < grids.length; grid_idx++) {
      ...
      for (var area_num = 0; area_num < areas.length; area_num++) {
        ...
        var tab_num = ".tab1";
        var area_id = String("#area" + areas[area_num] + "-ts" + ts);
...
function changeTab(tab_num) {
  ...
  d3.tsv("/assets/sensonly_full.txt").then( function(data) {
    for (var ts = 1; ts <= max_ts; ts++) {
      ...
      // document.querySelector(tab_num + ' #ts_output').innerHTML = metadata.ts;
    }) // END then
    // palette_picker();
};
...
for (var instance_num = 1; instance_num <= max_ts; instance_num++) {
  instance_id = String("#instance-" + instance_num);
  drawInstance(".tab1", instance_id, instance_num);
  drawInstance(".tab2", instance_id, instance_num);
}
